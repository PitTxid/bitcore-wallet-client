...
var coinList = ['btc', 'bch', 'xsg', 'zec', 'dash', 'ltc', 'zen', 'kmd']
/**
...
API.prototype.getFeeLevels = function(coin, network, cb) {
  ...
  $.checkArgument(coin || _.includes(coinList, coin));
...
API.prototype.createWallet = function(walletName, copayerName, m, n, opts, cb) {
  ...
  var coin = opts.coin || 'btc';
  if (!_.includes(coinList, coin)) return cb(new Error('Invalid coin'));
...
API.prototype.getBalance = function(opts, cb) {
  ...
  if (opts.coin) {
    if (!_.includes(coinList, opts.coin)) return cb(new Error('Invalid coin'));
